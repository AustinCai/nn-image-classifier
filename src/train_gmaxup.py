...
def run_gmaxup_on_sample(sample_num, x, y, loss_func, model, augmented_batch, args, 
    ...
    for c in range(args.choices): # number of layers to sample, C
        ...
        x_aug_img, used_augments = apply_transormations(operation_idxs, x_img, args.range) 
        ...
        if writer and args.test:
            visualize.show_images(writer, x_aug_tensor, 1, 
                title="Image_{}-opidx_{}_{}-{}_loss".format(
                    c, 
                    augmentations.augment_list(args.range)[operation_idxs[0]][3], 
                    augmentations.augment_list(args.range)[operation_idxs[1]][3], 
                    round(loss,2)))
            print("operation_1: {}".format(augmentations.augment_list(args.range)[operation_idxs[0]][3]))
            print("operation_2: {}".format(augmentations.augment_list(args.range)[operation_idxs[1]][3]))
        ...
        heapq.heappush(xy_tuple_list_by_loss, (loss, sample_num+c, used_augments, xy_tuple))

    for loss, img_id, used_augments, xy_tuple in heapq.nlargest(args.multiplier, xy_tuple_list_by_loss):
...
def main(args):
    ...
    model, optimizer, epoch, loss = training.load_model(model_path, model, optimizer)
    loss_func = torch.nn.CrossEntropyLoss()

    save_str = "gmaxup_{}-{}-{}l-{}s-{}".format(
        args.dataset, args.range, args.layers, args.multiplier, 
        datetime.datetime.now().strftime("%H:%M-%-m.%d.%y"))

    writer = SummaryWriter(Path(__file__).parent / '../runs/{}'.format(save_str))
    ...
    for batch_num, curr_batch_str in enumerate(dataset_batches):
        images, labels = data_loading.load_data_by_path(
            Path(__file__).parent / '../data/{}/{}'.format(args.dataset, curr_batch_str))        
    ...
    with open(Path(__file__).parent / "../gmaxup_data/{}".format(save_str), 'wb') as handle:
